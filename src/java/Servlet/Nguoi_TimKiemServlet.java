 ...
 * @author hayghe
 */
public class Nguoi_TimKiemServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String svl_userId=request.getParameter("USERID").trim();
        int userId=0;
        if(svl_userId != ""){
            userId=Integer.parseInt(svl_userId);
        }
//        System.out.println("timNguoi: "+ userId + svl_userId);
        int returnCode=0;
        Connection conn = DBConnectionUser.getConnection();
        List<VCBNguoiTimKiemEntity> entitys = new ArrayList<VCBNguoiTimKiemEntity>();
        try {
            VCBDomain CV = new VCBDomain(conn);
            entitys = CV.nguoi_TimKiemDomain(userId);
        ...
        try {
            ...
            for(VCBNguoiTimKiemEntity item:entitys) {
