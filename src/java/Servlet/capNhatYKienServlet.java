 ...
 * @author hayghe
 */
public class capNhatYKienServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String param_yKienId = pdp.getParameter("yKienId").trim();
        int yKienId = param_yKienId.length() > 0 ? Integer.parseInt(param_yKienId) : -1;
        int returnCode=0;
        Connection conn = DBConnectionUser.getConnection();
        VCBDomain vcb = new VCBDomain(conn);
        try {
                returnCode = vcb.capNhatYKienDomain(yKienId,param_noiDung);
