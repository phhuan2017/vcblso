 ...
 * @author hayghe
 */
public class themChucVuServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        /*maChucVu,tenChucVu,chucNangNhiemVu,User,trangThai*/
        String param_maChucVu = pdp.getParameter("maChucVu").trim();
        int maChucVu=0;
        if(param_maChucVu!=""){
            maChucVu=Integer.parseInt(param_maChucVu);
        }
        String param_tenChucVu = pdp.getParameter("tenChucVu").trim();
        String param_chucNangNhiemVu = pdp.getParameter("chucNangNhiemVu").trim();
        String param_User = pdp.getParameter("User").trim();
        int User=0;
        if(param_User!=""){
            User=Integer.parseInt(param_User);
        }
        String param_trangThai = pdp.getParameter("trangThai").trim();
        int trangThai=0;
        if(param_trangThai!=""){
            trangThai=Integer.parseInt(param_trangThai);
        }
//        System.out.println("vao day roi nhe " + maChucVu);
        int returnCode=0;
        Connection conn = DBConnectionUser.getConnection();
        VCBDomain vcb = new VCBDomain(conn);
        try {
            returnCode = vcb.themChucVuDomain(maChucVu,param_tenChucVu,param_chucNangNhiemVu,User,trangThai);
