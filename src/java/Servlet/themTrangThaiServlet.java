 ...
 * @author hayghe
 */
public class themTrangThaiServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String param_tenTrangThai = pdp.getParameter("tenTrangThai").trim();
        String param_maTrangThai = pdp.getParameter("maTrangThai").trim();
        int maTrangThai=Integer.parseInt(param_maTrangThai);
        int user=Integer.parseInt(pdp.getParameter("user").trim());
//        System.out.println("vao day chua: "+ maTrangThai);
        int returnCode=0;
        Connection conn = DBConnectionUser.getConnection();
        VCBDomain vcb = new VCBDomain(conn);
        try {
            returnCode = vcb.themTrangThaiDomain(param_tenTrangThai,user,maTrangThai);
