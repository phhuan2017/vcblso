 ...
 * @author hayghe
 */
public class timChucVuServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String tenChucVu=request.getParameter("tenChucVu").trim();
        String maChucVu=request.getParameter("maChucVu").trim();
        int chucVuID=maChucVu.length() > 0 ? Integer.parseInt(maChucVu) : -1;
        int returnCode=0;
        Connection conn = DBConnectionUser.getConnection();
        List<VCBChucVuEntity> entitys = new ArrayList<VCBChucVuEntity>();
        try {
            VCBDomain CV = new VCBDomain(conn);
            entitys = CV.timChucVuDomain(tenChucVu,chucVuID);
        ...
        try {
            ...
            for(VCBChucVuEntity item:entitys) {
