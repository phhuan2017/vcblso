 ...
 * @author hayghe
 */
public class timTrangThaiServlet extends HttpServlet {
...
//        System.out.println("vao day chua: "+ MOTA);
        int returnCode=0;
        Connection conn = DBConnectionUser.getConnection();
        List<VCBTrangThaiEntity> entitys = new ArrayList<VCBTrangThaiEntity>();
        try {
            VCBDomain CV = new VCBDomain(conn);
            entitys = CV.timTrangThaiDomain(MOTA);
        ...
        try {
            ...
            for(VCBTrangThaiEntity item:entitys) {
